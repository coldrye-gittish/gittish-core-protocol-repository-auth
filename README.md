# gittish-core-protocol-repository-auth

This package defines the service interface that is responsible for managing the
ACLs of a given repository and also for authorizing the operations on a given
repository based on these ACLs.

The service supports the following use cases

* check whether a given actor has the specified permission for a specific
  repository
* retrieve the ACL for a specific repository
* grant, deny and revoke permissions for specific actors for a specific
  repository
...
```
cd <WORKSPACE_ROOT>/gittish-core-protocol-repository-auth
./configure
```
